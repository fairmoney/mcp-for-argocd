...
test('supported filters are sent as the query parameters ArgoCD accepts', async (t) => {
  const requests = installFetch(FIXTURE);
  t.after(() => mock.restoreAll());
  const client = new ArgoCDClient(BASE_URL, 'tok');
  ...
  await client.listApplications({
    ...
    repo: 'https://git.example.com/charts.git',
    appNamespace: 'argocd'
  });

  const params = requests[0].searchParams;
  assert.equal(params.get('projects'), 'platform');
  assert.equal(params.get('selector'), 'team=payments');
  assert.equal(params.get('repo'), 'https://git.example.com/charts.git');
  assert.equal(params.get('appNamespace'), 'argocd');
});

test('no filters produce a bare request', async (t) => {
  const requests = installFetch(FIXTURE);
  t.after(() => mock.restoreAll());
  const client = new ArgoCDClient(BASE_URL, 'tok');
